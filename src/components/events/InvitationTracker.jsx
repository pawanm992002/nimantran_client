...
const InvitationTracker = () => {
    ...
    const filteredData = selectedStatus === 'All' ? tableData : tableData.filter(item => item.status === selectedStatus);
                ...
                <thead className="bg-gray-100 text-gray-600 text-left">
                    <tr>
                        ...
                        <th className="border px-4 py-2 box-border">
                            <label >Status : </label>
                            <select className="border px-4 py-1  box-border rounded-md" onChange={(e) => handleFiltersStatusChange(e)}>
                    ...
                    {filteredData.map((row, index) => (
                        <tr key={index}>
                            ...
                            <td className="border px-4 py-2 box-border">
                                <button className="bg-gray-500 px-4 rounded-sm py-0.5 text-gray-100" onClick={() => setOpenModel(true)}>
                                    View
                                </button>
                            </td>
                        </tr>
